...
use AymanElarian\Extensions\SqlServer\SpatialServiceProvider;
use AymanElarian\Extensions\SqlServer\Types\GeometryCollection;
use AymanElarian\Extensions\SqlServer\Types\LineString;
use AymanElarian\Extensions\SqlServer\Types\MultiPoint;
use AymanElarian\Extensions\SqlServer\Types\MultiPolygon;
...
class SpatialTest extends BaseTestCase
{
    ...
    public function createApplication()
    {
        ...
        $app['config']->set('database.connections.sqlsrv.database', 'spatial_test');
        $app['config']->set('database.connections.sqlsrv.username', 'root');
        $app['config']->set('database.connections.sqlsrv.password', '');
        $app['config']->set('database.connections.sqlsrv.modes', [
            'ONLY_FULL_GROUP_BY',
            'STRICT_TRANS_TABLES',
            'NO_ZERO_IN_DATE',
            'NO_ZERO_DATE',
    ...
    public function setUp()
    {
        ...
        $this->after_fix = $this->issqlsrv8AfterFix();
    ...
    // sqlsrv 8.0.4 fixed bug #26941370 and bug #88031
    private function issqlsrv8AfterFix()
    {
        $results = DB::select(DB::raw('select version()'));
        $sqlsrv_version = $results[0]->{'version()'};

        return version_compare($sqlsrv_version, '8.0.4', '>=');
    }
    ...
    public function testSpatialFieldsNotDefinedException()
    {
        ...
        $this->assertException(\AymanElarian\Extensions\SqlServer\Exceptions\SpatialFieldsNotDefinedException::class);
    ...
    //    $point = new Point(0, 0);
    //
    //    $linestring1 = \AymanElarian\Extensions\SqlServer\Types\LineString::fromWkt("LINESTRING(1 1, 2 2)");
    //    $linestring2 = \AymanElarian\Extensions\SqlServer\Types\LineString::fromWkt("LINESTRING(20 20, 24 24)");
    //    $linestring3 = \AymanElarian\Extensions\SqlServer\Types\LineString::fromWkt("LINESTRING(0 10, 10 10)");
    //
